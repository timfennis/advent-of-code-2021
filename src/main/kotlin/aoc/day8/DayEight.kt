...
private fun solveNote(note: Note): Int {
    val cf = note.patterns.first { it.length == 2 }
    val bd = note.patterns.first { it.length == 4 }.filterNot { it in cf }


    val a = find_segment_with_all_other_of_length(note, cf, 3)
    val g = find_segment_with_all_other_of_length(note, cf + a + bd, 6)
    val e = find_segment_with_all_other_of_length(note, cf + a + bd + g, 7)
    ...
    val f = find_segment_with_all_other_of_length(note, c.toString(), 2)
...
private fun fixSegments(input: String, mapping: Map<Char, Char>): List<Char> = input.map { mapping[it]!! }

private fun find_segment_with_all_other_of_length(note: Note, other: String, length: Int) =
    note.patterns
        .first { it.length == length && other.all { c -> it.contains(c) } }
        .filterNot { it in other }.single()

private fun List<Char>.toDecimal(): Int = when(this.sorted().joinToString("")) {
    "abcefg" -> 0
    "cf" -> 1
    "acdeg" -> 2
    "acdfg" -> 3
