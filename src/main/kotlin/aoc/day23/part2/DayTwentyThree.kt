...
private val nodes = listOf(
    ...
    Node("r1.1"), Node("r1.2"), Node("r1.3"), Node("r1.4"),
    Node("r2.1"), Node("r2.2"), Node("r2.3"), Node("r2.4"),
    Node("r3.1"), Node("r3.2"), Node("r3.3"), Node("r3.4"),
    Node("r4.1"), Node("r4.2"), Node("r4.3"), Node("r4.4"),
...
private val vertices = listOf(
    ...
    1 to listOf("r1.1", "r1.2", "r1.3", "r1.4").zipWithNext(),
    1 to listOf("r2.1", "r2.2", "r2.3", "r2.4").zipWithNext(),
    1 to listOf("r3.1", "r3.2", "r3.3", "r3.4").zipWithNext(),
    1 to listOf("r4.1", "r4.2", "r4.3", "r4.4").zipWithNext()
).flatMap { (cost, b) ->
    b.flatMap { (a, b) ->
        listOf(
...
private val desiredState = State(
    mapOf(
        ...
        node("r1.3") to Amphipod('A'),
        node("r1.4") to Amphipod('A'),
        node("r2.1") to Amphipod('B'),
        node("r2.2") to Amphipod('B'),
        node("r2.3") to Amphipod('B'),
        node("r2.4") to Amphipod('B'),
        node("r3.1") to Amphipod('C'),
        node("r3.2") to Amphipod('C'),
        node("r3.3") to Amphipod('C'),
        node("r3.4") to Amphipod('C'),
        node("r4.1") to Amphipod('D'),
        node("r4.2") to Amphipod('D'),
        node("r4.3") to Amphipod('D'),
        node("r4.4") to Amphipod('D'),
    ), 0
)

private fun Map<Node, Content>.isEmpty(node: Node) = this[node]!! == Empty
private fun Map<Node, Content>.isOccupied(node: Node) = !this.isEmpty(node)
...
internal data class State(val map: Map<Node, Content>, val cost: Int) {
    override fun toString() = """
        ...
        ###${map[node("r1.3")]}#${map[node("r2.3")]}#${map[node("r3.3")]}#${map[node("r4.3")]}###
        ###${map[node("r1.4")]}#${map[node("r2.4")]}#${map[node("r3.4")]}#${map[node("r4.4")]}###
          ######### 
    """.trimIndent()
        ...
        fun from(input: String): State {
            ...
            return State(
                mapOf(
                    ...
                    node("r1.2") to Amphipod('D'),
                    node("r1.3") to Amphipod('D'),
                    node("r1.4") to Amphipod(c[4]),
                    node("r2.1") to Amphipod(c[1]),
                    node("r2.2") to Amphipod('C'),
                    node("r2.3") to Amphipod('B'),
                    node("r2.4") to Amphipod(c[5]),
                    node("r3.1") to Amphipod(c[2]),
                    node("r3.2") to Amphipod('B'),
                    node("r3.3") to Amphipod('A'),
                    node("r3.4") to Amphipod(c[6]),
                    node("r4.1") to Amphipod(c[3]),
                    node("r4.2") to Amphipod('A'),
                    node("r4.3") to Amphipod('C'),
                    node("r4.4") to Amphipod(c[7]),
                ), 0
            )
        }
    }
...
private fun State.nextPossibleStates(): Set<State> {
    ...
    DP[currentState.hash] = outputSet
...
private fun Node.backingRoomList() = sequence {
    ...
    for (number in room.roomNumber..4) {
